		...
		ImGui::BeginChild(xorstr("###Textures"), ImVec2(180, 180), true, m_iImGuiStyle);
		{
			for (size_t i = 0; i < pTexutreList->size(); ++i)
			{
				...
				ImGui::BeginChild((xorstr("###TextureChild") + pCurrentTexture->m_sName).c_str(), ImVec2(175, 25), true, m_iImGuiStyle);
				{
					...
					DrawToolTip(fmt::format("Texture id: \"{}\" ", pCurrentTexture->m_iUid).c_str());


					ImGui::SameLine();
					ImGui::SetCursorPosX(152);
					if (ImGui::Button(((const char*)xorstr("X###RemoveButton") + pCurrentTexture->m_sName).c_str(), ImVec2(21.f, 21.f)))
					{
