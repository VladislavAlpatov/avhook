	...
	ImGui::BeginChild(xorstr("###FeatureCfg"), ImVec2(180, 80), true, m_iImGuiStyle);
	{
		...
		if (ImGui::Button(xorstr("Import###fi")))
		{
			if (CConfigLoader cfg(GlobalVars::g_AllSettings.m_Name.c_str(), &GlobalVars::g_AllSettings); cfg.LoadConfigFile(GlobalVars::g_AllSettings.m_Name))
				m_pMessageLineList->Add(fmt::format("Loaded settings config: \"{}\"", GlobalVars::g_AllSettings.m_Name.c_str()), 2000);
			else
				m_pMessageLineList->Add(fmt::format("\"{}\" does not exist.", GlobalVars::g_AllSettings.m_Name.c_str()), 2000);
		...
		if (ImGui::Button(xorstr("Import###fin")))
		{
			if (m_pMenuCfgName[0] != NULL)
			{
				if (std::ifstream file(std::string(m_pMenuCfgName) + xorstr(".avmcfg"), std::ios::binary); file.is_open())
				{
					...
					m_pMessageLineList->Add(fmt::format("Loaded menu config: \"{}\"", m_pMenuCfgName), 2000);

				}
				else
				{
					m_pMessageLineList->Add(fmt::format("\"{}.avmcfg\" does not exist.", m_pMenuCfgName), 2000);
				}
			}
			else
			{
				m_pMessageLineList->Add("Enter the file name.", 2000, ImColor(255, 0, 0));
			}
		}
		ImGui::SameLine();
		if (ImGui::Button("Export###fex"))
		{
			if (m_pMenuCfgName[0] != NULL)
			{

				std::ofstream file(std::string(m_pMenuCfgName) + ".avmcfg", std::ios::binary);
				file.write((const char*)ImGui::GetStyle().Colors, sizeof(ImGui::GetStyle().Colors));
				file.close();
				m_pMessageLineList->Add("Config successfully exported.", 2000, ImColor(0, 255, 0));
			}
			else
			{
